...
export const fetchCompanies = async (query) => {
    ...
    const allowedSorts = ["funded", "new", "trending"];
...
export const createCompany = async (body) => {
    ...
    let slug = baseSlug;
    let count = 1;
    while (await prisma.company.findUnique({
        where: {
            slug,
        },
    })) {
...
export const getCompanyFunding = async (slug) => {
    const company = await prisma.company.findUnique({
        where: {
            slug,
        },
...
export const getCompanyProducts = async (slug) => {
    const company = await prisma.company.findUnique({
        where: { slug },
        select: {
            id: true,
